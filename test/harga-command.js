'use strict';
/**
 * test/harga-command.js
 * `.harga` (alias `.saham`/`.crypto`/`.koin`/`.forex`/`.kurs`/`.emas`/`.gold`):
...
console.log('✓ 1. harga + 7 aliasnya kecatat di menu tools & kena limit');
...
(async () => {
  ...
  console.log('✓ 2. 8 alias -> 1 request ke api/tools/harga-saham, balasan seragam');
  ...
  const aapl = ctxHarga('harga', ['aapl']);
  ...
  console.log('✓ 4. `.harga aapl` -> langsung AAPL, tanpa retry .JK');
  ...
  assert.ok(/BBCCA|Ketik \.harga/.test(t), 'harus ngarahin ke .harga <simbol> buat detail');
  ...
  console.log('✓ 6. `.harga` tanpa argumen -> 5 kelompok aset (USD/IDR, logam, indeks, forex, crypto, saham)');

  // ── 6b. API nggak balikin hasil → jangan kirim pesan kosong ────────────────
  panggilan.length = 0;
  jawab = () => ({ data: {} });
  const blm = ctxHarga('harga', []);
  ...
  panggilan.length = 0;
  jawab = () => tolak();
  const hantu = ctxHarga('harga', ['zzzznope']);
